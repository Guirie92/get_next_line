...
/*   Updated: 2025/10/23 22:24:37 by guillsan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

static size_t	process_reminder(char *s_buf, t_tmpbuf *tmp, t_gnl_data *data)
{
	if (!*s_buf)
		return (NL_NOT_FOUND);
...
static ssize_t
	process_final_line(char **s_buf, t_tmpbuf *tmp, t_gnl_data *data)
{
	data->line = malloc((data->len + 1) * sizeof(char));
	if (!(data->line))
		return (MEM_FAIL);
...
static ssize_t	process_line(char **s_buf, t_tmpbuf *tmp, t_gnl_data *data)
{
	if (check_alloc(tmp, data->len) == MEM_FAIL)
		return (MEM_FAIL);
...
static ssize_t
	process_loop(char **s_buf, t_tmpbuf *tmp, t_gnl_data *data, int fd)
{
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	if (init_data(&s_buf[fd], &tmp, &data) == MEM_FAIL)
		return (NULL);
	data.ret_code = process_reminder(s_buf[fd], &tmp, &data);
	if (data.ret_code == MEM_FAIL)
		return (cleanup(&s_buf[fd], &tmp, &data));
	if (data.ret_code == NL_FOUND)
		return (data.line);
	data.bytes_read = read(fd, s_buf[fd], BUFFER_SIZE);
	if (data.bytes_read == -1)
		return (cleanup(&s_buf[fd], &tmp, &data));
	data.total_bytes = tmp.idx + data.bytes_read;
	data.len = tmp.idx;
	data.ret_code = process_loop(&s_buf[fd], &tmp, &data, fd);
	if (data.ret_code == NL_FOUND)
		return (data.line);
	return (cleanup(&s_buf[fd], &tmp, &data));
}
